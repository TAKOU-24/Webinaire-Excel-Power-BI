...
export default function App() {
  ...
  // Field → entry.xxxxx mapping (based on the order provided — verify with a
  // pre-filled Google Form link before fully trusting this mapping)
  // Confirmed entry IDs (verified via pre-filled Google Form link).
  ...
  // Submits to Google Forms via a hidden iframe + native form POST, instead
  // of fetch(). This mimics a real HTML form submission (like clicking
  // "Envoyer" on the actual Google Forms page) rather than a JS network
  // call — more resilient to ad/privacy blockers that specifically target
  // fetch()/XHR calls to third-party domains without touching native form
  // submissions.
  const submitToGoogleForms = (values) => {
    return new Promise((resolve) => {
      console.log("[GoogleForms] Starting submission with values:", values);
      ...
      formEl.method = "POST";
      // TEMPORARY DIAGNOSTIC: navigate the CURRENT tab directly to Google's
      // response (no target = same window), so nothing can be popup-blocked.
      // This sacrifices the /merci redirect for this one test.
      // Revert to `formEl.target = IFRAME_ID;` once the issue is fixed.
      ...
      Object.entries(GOOGLE_FORM_ENTRIES).forEach(([field, entryId]) => {
        ...
        console.log(`[GoogleForms]   ${entryId} (${field}) = "${values[field] || ""}"`);
      });

      document.body.appendChild(formEl);
      console.log("[GoogleForms] Calling formEl.submit() targeting iframe:", IFRAME_ID);
      formEl.submit();
      console.log("[GoogleForms] submit() called, waiting before cleanup...");

      // Cross-origin iframe means we can't inspect the response — give the
      // browser time to actually dispatch the request before cleaning up.
      // Removing the form synchronously right after submit() can, in some
      // browsers, interrupt the pending navigation before it's sent.
      setTimeout(() => {
        if (formEl.parentNode) document.body.removeChild(formEl);
        console.log("[GoogleForms] Cleanup done, resolving.");
        resolve();
      }, 800);
    });
  };
      ...
      <section className="max-w-3xl mx-auto px-6 pt-16 md:pt-24 pb-10 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-8">
          <span className="flex items-center gap-2 text-sm px-4 py-2 rounded-full" style={{ color: COLORS.text, background: COLORS.surface2, border: `1px solid ${COLORS.border}` }}>
            <Calendar size={15} color={COLORS.cyan} /> 13 août 2026
          </span>
          <span className="flex items-center gap-2 text-sm px-4 py-2 rounded-full" style={{ color: COLORS.text, background: COLORS.surface2, border: `1px solid ${COLORS.border}` }}>
      ...
      <section className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="space-y-4">
          {[
            ...
            { q: "Que se passe-t-il si je ne suis pas disponible le 13 août à 20h ?", a: "Inscrivez-vous quand même : le replay vous sera envoyé automatiquement par e-mail après l'événement." },
