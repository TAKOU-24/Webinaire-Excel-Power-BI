  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Object.entries(GOOGLE_FORM_ENTRIES).forEach(([field, entryId]) => {
        params.append(entryId, form[field] || "");
      });

      // Google Forms doesn't allow reading the response cross-origin (no-cors),
      // so we can't inspect res.ok — a successful network send (no thrown
      // exception) is treated as success, matching the standard pattern for
      // submitting to Google Forms from client-side JS without redirecting
      // the user to the Google Forms interface.
